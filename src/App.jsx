...
const exportCSV = (reservations) => {
  const rows = [["Booth", "Department", "Name", "Student ID", "Group/Project", "Reserved At"]];
  Object.entries(reservations)
    .sort(([a], [b]) => a.localeCompare(b, undefined, { numeric: true }))
    .forEach(([id, r]) => {
      rows.push([id, r.dept, r.name, r.studentId, r.group || "", r.ts ? new Date(r.ts).toLocaleString() : ""]);
    });
...
// ─── Booth ────────────────────────────────────────────────────────────────────
function Booth({ id, reservations, onSelect, isAdmin, onAdminDelete }) {
  ...
  return (
    <div
      onClick={() => {
        if (isAdmin && reserved) { onAdminDelete(id); return; }
      ...
      title={
        reserved
          ? isAdmin
            ? `${id} — ${res.name} (${res.studentId}) · Click to cancel`
            : `${id} — Already reserved`
          : `${id}: ${cfg.label} — click to reserve`
      }
      style={{
        width: BS, height: BS, margin: BG,
        ...
        cursor: reserved ? (isAdmin ? "pointer" : "default") : "pointer",
        transform: hov ? "scale(1.18)" : "scale(1)",
        boxShadow: hov ? `0 4px 14px ${reserved ? "#ef444444" : cfg.color + "55"}` : "none",
        zIndex: hov ? 20 : 1,
        position: "relative",
    ...
    >
      {reserved ? (
        <>
          <span style={{ fontSize: 13, lineHeight: 1 }}>{isAdmin && hov ? "✕" : "✓"}</span>
          <span style={{ fontSize: 7 }}>{id}</span>
        </>
      ) : <span>{id}</span>}
    </div>
  );
...
// ─── Field map ────────────────────────────────────────────────────────────────
function FieldMap({ field, reservations, onSelect, isAdmin, onAdminDelete }) {
  const f = FIELDS[field];
  const rb = (id) => (
    <Booth key={id} id={id} reservations={reservations}
      onSelect={onSelect} isAdmin={isAdmin} onAdminDelete={onAdminDelete} />
  );
  ...
  return (
    <div style={{ overflowX: "auto", paddingBottom: 8 }}>
      <div style={{
        ...
        border: "2px solid #e2e8f0", borderRadius: 10, padding: 8,
        ...
        <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-end" }}>
          ...
          <div style={{
            flex: 1, minWidth: 14, height: BS + BG * 2,
            display: "flex", alignItems: "center", justifyContent: "center",
          }}>
            <div style={{
              background: "#fef9c3", border: "1px dashed #ca8a04",
              borderRadius: 4, padding: "2px 8px",
              fontSize: 9, fontWeight: 700, color: "#92400e", whiteSpace: "nowrap",
            }}>ENTRANCE</div>
          </div>
        ...
        <div style={{ display: "flex", alignItems: "stretch" }}>
          ...
          <div style={{
            flex: 1, minWidth: 160, margin: 4, borderRadius: 7,
            background: "linear-gradient(155deg, #4ade80 0%, #22c55e 45%, #15803d 100%)",
            display: "flex", alignItems: "center", justifyContent: "center",
            position: "relative", overflow: "hidden",
          }}>
            {[...Array(6)].map((_, i) => (
              <div key={i} style={{
                position: "absolute", top: `${10 + i * 16}%`,
                left: 0, right: 0, height: 1, background: "rgba(255,255,255,0.12)",
              }} />
            ))}
            <div style={{
              background: "rgba(0,0,0,0.22)", color: "#fff",
              borderRadius: 7, padding: "10px 18px", textAlign: "center",
              backdropFilter: "blur(2px)",
            }}>
              <div style={{ fontSize: 16, fontWeight: 900, letterSpacing: "-0.02em" }}>{f.name}</div>
    ...
    <div onClick={e => e.target === e.currentTarget && onClose()} style={{
      ...
      <div style={{
        ...
        <div style={{ display: "flex", gap: 10, marginTop: 22 }}>
          ...
          <button
            onClick={() => valid && !loading && onConfirm(boothId, { name: name.trim(), studentId: sid.trim(), group: group.trim() })}
            disabled={!valid || loading}
            style={{
              flex: 2, padding: "11px 0", border: "none",
              borderRadius: 9, fontSize: 14, fontWeight: 700, color: "#fff",
              background: valid && !loading ? cfg.color : "#cbd5e1",
              cursor: valid && !loading ? "pointer" : "not-allowed",
              transition: "background 0.2s",
            }}
          >{loading ? "Saving…" : "Confirm Reservation"}</button>
        </div>
      </div>
    </div>
  );
    ...
    <div onClick={e => e.target === e.currentTarget && onClose()} style={{
      ...
      <div style={{ background: "#fff", borderRadius: 14, padding: 28, width: "100%", maxWidth: 340, boxShadow: "0 30px 90px rgba(0,0,0,0.3)" }}>
        ...
        <input
          ...
          onKeyDown={e => e.key === "Enter" && (pw === ADMIN_PASSWORD ? onSuccess() : setErr(true))}
          placeholder="Admin password"
          style={{
            width: "100%", padding: "10px 12px",
            border: `1.5px solid ${err ? "#ef4444" : "#e2e8f0"}`,
            borderRadius: 8, fontSize: 14, outline: "none", marginBottom: 6,
          }}
          autoFocus
        />
    ...
    <div style={{
      ...
      <div style={{
        ...
        <div style={{ padding: "18px 22px", borderBottom: "1px solid #e2e8f0", display: "flex", alignItems: "center", gap: 12 }}>
          ...
          <button onClick={() => exportCSV(reservations)} style={{
          ...
          <button onClick={() => { if (confirm("Delete ALL reservations? This cannot be undone.")) onDeleteAll(); }} style={{
        ...
        <div style={{ padding: "12px 22px", borderBottom: "1px solid #f1f5f9" }}>
          <input
            ...
            placeholder="Search by booth, name, student ID, or department…"
            style={{
            ...
            return (
              <div key={id} style={{
                ...
                <div style={{
                  ...
                  display: "flex", alignItems: "center", justifyContent: "center",
                  fontSize: 10, fontWeight: 800, color: cfg.text,
                }}>{id}</div>
                <div style={{ flex: 1, minWidth: 0 }}>
                  <div style={{ fontSize: 14, fontWeight: 700, color: "#1e293b" }}>{res.name}</div>
                  <div style={{ fontSize: 12, color: "#64748b" }}>
                    {res.studentId}{res.group ? ` · ${res.group}` : ""}
                  </div>
                  {res.ts && (
                ...
                <div style={{
                  ...
                  background: cfg.bg, borderRadius: 5, padding: "3px 8px", flexShrink: 0,
                }}>{res.dept}</div>
                <button onClick={() => { if (confirm(`Cancel reservation for booth ${id}?`)) onDelete(id); }} style={{
                  padding: "6px 12px", border: "1px solid #fca5a5", borderRadius: 7,
                  background: "#fff", cursor: "pointer", fontSize: 12, fontWeight: 600, color: "#ef4444", flexShrink: 0,
                }}>Cancel</button>
              </div>
            );
          })}
...
function Toast({ msg, type }) {
  ...
  return (
    <div style={{
      ...
      borderRadius: 10, fontSize: 14, fontWeight: 600, zIndex: 3000,
      boxShadow: "0 8px 30px rgba(0,0,0,0.3)", display: "flex",
      alignItems: "center", gap: 8, whiteSpace: "nowrap",
    }}>
      <span style={{ color: type === "error" ? "#fecaca" : "#4ade80" }}>
        {type === "error" ? "✕" : "✓"}
      </span>
      {msg}
    </div>
...
export default function App() {
  ...
  useEffect(() => {
    const unsub = onSnapshot(collection(db, "reservations"), (snap) => {
      ...
      console.error(err);
      showToast("Connection error. Check your Firebase rules.", "error");
      setLoading(false);
    });
    return () => unsub();
  }, []);

  // Secret admin trigger: click the title 5 times
  ...
  const handleConfirm = async (boothId, data) => {
    setSaving(true);
    try {
      await setDoc(doc(db, "reservations", boothId), {
        ...data,
        dept: getDept(boothId),
        ts: Date.now(),
      });
  ...
  return (
    <div style={{ fontFamily: "'Segoe UI', system-ui, sans-serif", minHeight: "100vh", background: "#f1f5f9" }}>
      ...
      {/* Header */}
      <div style={{
        background: "linear-gradient(135deg, #0c2340 0%, #1a3f6f 100%)",
        color: "#fff", padding: "18px 24px",
        display: "flex", justifyContent: "space-between", alignItems: "center", flexWrap: "wrap", gap: 12,
      }}>
        <div onClick={handleTitleClick} style={{ cursor: "default", userSelect: "none" }}>
          <div style={{ fontSize: 10, fontWeight: 700, color: "#93c5fd", letterSpacing: "0.12em", textTransform: "uppercase", marginBottom: 5 }}>
            Class of 2026 · Pre-Graduation Event
          </div>
          <h1 style={{ margin: 0, fontSize: 20, fontWeight: 900, letterSpacing: "-0.02em" }}>
            Booth Reservation Map
          </h1>
        </div>
        <div style={{ display: "flex", gap: 10, alignItems: "center" }}>
          ...
          {[{ label: "Field A", s: sA }, { label: "Field B", s: sB }].map(({ label, s }) => (
            <div key={label} style={{
              background: "rgba(255,255,255,0.1)", borderRadius: 9,
              padding: "10px 16px", textAlign: "center", minWidth: 88,
            }}>
              ...
              <div style={{ fontSize: 9, color: "#94a3b8", marginTop: 1 }}>{s.res} reserved / {s.total}</div>
            </div>
          ))}
        </div>
      </div>
      ...
      <div style={{
        ...
        {["A", "B"].map(f => {
          ...
          return (
            <button key={f} onClick={() => setActiveField(f)} style={{
              ...
              borderBottom: active ? "3px solid #1a3f6f" : "3px solid transparent",
              background: "none", cursor: "pointer",
              fontSize: 14, fontWeight: active ? 700 : 500,
              color: active ? "#1a3f6f" : "#64748b",
              display: "flex", alignItems: "center", gap: 8, transition: "all 0.15s",
            }}>
              {FIELDS[f].name} · {FIELDS[f].subtitle}
              <span style={{
                background: active ? "#1a3f6f" : "#e2e8f0",
        ...
        {isAdmin && (
          <div style={{ marginLeft: "auto", alignSelf: "center", padding: "0 8px" }}>
            <div style={{
              background: "#fef9c3", border: "1px solid #fcd34d",
              borderRadius: 6, padding: "4px 10px", fontSize: 11, fontWeight: 700, color: "#92400e",
            }}>⚠ Admin mode — click reserved booth to cancel</div>
          </div>
        )}
      </div>
        ...
        <div style={{ display: "flex", flexWrap: "wrap", gap: 5, marginBottom: 14 }}>
          {Object.entries(DEPT).filter(([k]) => k !== "NONE").map(([key, cfg]) => (
            <div key={key} style={{
              ...
              <div style={{ width: 9, height: 9, borderRadius: 2, background: cfg.color, flexShrink: 0 }} />
          ...
          <div style={{ display: "flex", alignItems: "center", gap: 5, background: "#f3f4f6", border: "1px solid #d1d5db", borderRadius: 6, padding: "4px 9px" }}>
            <span style={{ fontSize: 12, lineHeight: 1, color: "#9ca3af" }}>✓</span>
      ...
      {showAdminLogin && (
        <AdminLogin
          ...
          onSuccess={() => { setIsAdmin(true); setShowAdminLogin(false); showToast("Admin mode enabled."); }}
        />
      )}
      {showAdminPanel && (
        <AdminPanel
